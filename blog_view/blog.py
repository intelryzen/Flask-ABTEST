...
def set_email(): 
    ...
    return redirect(url_for('blog_abtest.blogfullstack1')) 
...
@blog_abtest.route('/blogfullstack1')
def blogfullstack1():
    ...
    if current_user.is_authenticated: # 내부적으로 blog_abtest 의 load_user(user_id) 함수를 호출해서 인증된 사용자인지 확인함. current_user 객체는 User.get() 으로 반환된 User 의 모든 속성을 가짐. user_id 는 사용자의 http request 에서 추출함.
        ...
        return render_template(webpage_name, user_email=current_user.user_email) # render_template 은 jinja2 라이브러리로 html 의 user_email 의 값을 대입 후 클라이언트에 그 html 을 전송
    else:
